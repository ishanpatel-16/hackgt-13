  ...
  return (
    <section className="panel details-panel" aria-labelledby="node-details-heading">
      ...
      <div className="details-body">
        ...
        <div className="node-reports">{reports.map(incident => <button key={incident.id} className={`incident ${incident.type.toLowerCase()}`} onClick={() => onSelectIncident(incident.id)}><span className="incident-top"><strong>{incident.type}</strong><span className={`incident-status ${incident.status.toLowerCase()}`}>{incident.status}</span></span><span>{incident.people} {incident.people === 1 ? 'person' : 'people'} · Access Point {incident.node}</span><span className="incident-details">SOS / {incident.id} · {incident.age} →</span></button>)}</div>

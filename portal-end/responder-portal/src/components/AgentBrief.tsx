  ...
  return (
    <section className="panel agent-panel" aria-labelledby="agent-heading">
      <div className="panel-heading">
        ...
        <span className="small-label">Synthesized from {brief.reportCount} reports + network status</span>
      </div>
      <div className="agent-content">
        <div className="brief-summary">
          <h3>Situation Brief</h3>
          <p className="agent-insight">{brief.insight}</p>
          <ul className="brief-highlights">{brief.highlights.map(highlight => <li key={highlight}>{deviceName(highlight)}</li>)}</ul>
          <p className="agent-explanation">{deviceName(brief.summary)}</p>
        </div>
        <div className="agent-support"><h3>Related Signals</h3><ul>{brief.signals.map(signal => <li key={signal}>{deviceName(signal).replace('Mesh route operational', 'Emergency message path operational')}</li>)}</ul></div>

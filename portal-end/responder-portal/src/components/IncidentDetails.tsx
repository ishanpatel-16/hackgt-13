  ...
  return (
    <section className={`panel details-panel ${incident.type.toLowerCase()}`} aria-labelledby="details-heading">
      ...
      <div className="details-body">
        ...
        <dl className="detail-facts"><div><dt>Location</dt><dd>{incident.placeName ?? incident.location}<br />{incident.locationDetail}</dd></div></dl>
        <h4>Primary response</h4><p className="detail-route">{getPrimaryResponse(incident.type)}</p>
        <h4>Report</h4><blockquote>“{incident.report}”</blockquote>
        <p className="detail-route">Reported {incident.reported}</p>
        ...
        <dl className="detail-facts"><div><dt>Received via</dt><dd>Access Point {incident.node}</dd></div><div><dt>Coordinates</dt><dd>{incident.location}</dd></div></dl>
        <h4>How this report reached us</h4><p className="detail-route">{incident.path.map(deviceName).join(' → ')}</p>
        {showReports && <div id="source-reports" className="source-reports"><h4>Source Report · 1</h4><p>SOS / {incident.id} · Access Point {incident.node} · {incident.reported}</p><p>{incident.report}</p><p>No other reports have been linked to this incident.</p></div>}
      </div>
    </section>
  )
}
